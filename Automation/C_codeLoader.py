...
#Creates the header row. Inserts the link to the template, fills in light blue, and freezes it
    ...
    if(sheet_type == 'base'):
        for i in range(0, len(alphabet), 1):
            ...
            if(i%2==0):
                current_worksheet[letter+'1'].value = '=[ObfuscationCategorization.xlsx]Template!'+letter+'30'
...
root_dir = '../ObfuscationDatabase/Compiled_Code'

# Load the Excel spreadsheet
current_workbook = '\\CompiledTEST.xlsx'#'\\PaLM_Compiled.xlsx'#'\\Jurassic_2_Compiled.xlsx'#'\\ChatGPT.xlsx'#'\\Q3Test.xlsx'#'\\Q2Test.xlsx'#'\\PaLM_Q1.xlsx'#'\\JTEST.xlsx'##'\\ObfuscationCategorization.xlsx''\\LM2.xlsx'
...
#Load Obfuscation Compiled Code
    if(root.split('\\')[-1][0] == 'C' and root.split('\\')[-1] != 'Compiled_Code'):
